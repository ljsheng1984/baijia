...
var btns = document.querySelectorAll(".addcart"); //所有的购物车按钮

//约定好用名称为datas的cookie来存放购物车里的数据信息  datas里所存放的就是一个json字符串
var listStr = cookieObj.get("shopdatas");
...
for(var i = 0, len = btns.length; i < len; i++) {
    btns[i].onclick = function() {
        var arrs = document.getElementsByClassName("shop");//获取所有子节点
        var gid = arrs[0].innerHTML;
        if (checkObjByGid(gid)) {
            listObj = updateObjById(gid, 1)
        } else {
            var type = arrs[1].innerHTML;
            var imgSrc = arrs[2].firstElementChild.src;;
            var pName = arrs[3].innerHTML;
            var price = arrs[4].innerHTML;
            var obj = {
                gid: gid,
                pImg: imgSrc,
                pName: pName,
