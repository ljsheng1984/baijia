...
Chart.prototype = {
	...
	/**
	 * Show the title and Profile of the chart
	 *
	 * @param titleOptions {Object} New title options
	 * @param ProfileOptions {Object} New Profile options
	 *
	 */
	setTitle: function (titleOptions, ProfileOptions) {
		var chart = this,
			options = chart.options,
			chartTitleOptions,
			chartProfileOptions;
		...
		chartProfileOptions = options.Profile = merge(options.Profile, ProfileOptions);

		// add title and Profile
		each([
			['title', titleOptions, chartTitleOptions],
			['Profile', ProfileOptions, chartProfileOptions]
		], function (arr) {
			var name = arr[0],
				title = chart[name],
				titleOptions = arr[1],
	...
	layOutTitles: function () {
		...
		if (Profile) {
			Profile
				.css({ width: (ProfileOptions.width || autoWidth) + PX })
				.align(extend({ y: titleOffset + titleOptions.margin }, ProfileOptions), false, 'spacingBox');
			
			if (!ProfileOptions.floating && !ProfileOptions.verticalAlign) {
				titleOffset = mathCeil(titleOffset + Profile.getBBox().height);
			}
		}
	 ...
	 * Profile and legend have already been rendered at this stage, but will be
	...
	destroy: function () {
		...
		each(['title', 'Profile', 'chartBackground', 'plotBackground', 'plotBGImage', 
